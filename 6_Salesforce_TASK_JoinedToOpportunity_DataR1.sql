...
-- REQUIRED TABLE JOINS
	--ACCOUNT LEFT JOIN OPPORTUNITY USING COMMON ACCOUNT CODES 001
		  LEFT JOIN [SFDCReplicaDB].[dbo].[opportunity] as opp
			ON opp.AccountID = task.AccountId

			WHERE (opp.[Type] = 'New Sale' or opp.[Type] = 'Migration')
			AND YEAR(opp.CloseDate) > 2013 and opp.Close_Reasons__c not like 'Duplicate Opportunity' AND (opp.StageName = 'Closed Lost' or opp.StageName = 'Closed Won') and DATEDIFF(d,opp.CreatedDate,opp.CloseDate) > 1
