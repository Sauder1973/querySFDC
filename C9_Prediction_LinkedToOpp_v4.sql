...
SELECT 
  ...
  -- REQUIRED TABLE JOINS
	...
	-- Acquire Segment Data From Account		
			JOIN [SFDCReplicaDB].[dbo].[account] as acc
				ON acc.[Id] = opp.[AccountId]

	-- Acquire Product Type Data from Opportunity Line Item
	...
	(
		...
		WHERE lineItem.[Product_Group__c] = 'Powerpay'
	) as powerPayOpps
				 
				ON powerPayOpps.[OpportunityID] = opp.[Id]
